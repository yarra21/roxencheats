...
export const getSystemInfo = async (): Promise<SystemInfo> => {
  try {
    if (typeof window === "undefined") {
      return { ip: "server-side", browser: "unknown", os: "unknown", device: "unknown" }
    }

    // IP adresini al (gerçek)
    let ip = "Bilinmiyor"
    try {
      ...
      // Alternatif IP servisi dene
      try {
        const ipResponse = await fetch("https://api.ipgeolocation.io/getip")
        const ipData = await ipResponse.json()
        ip = ipData.ip
      } catch (error) {
        console.error("Alternatif IP servisi de başarısız:", error)
      }
    }

    // Tarayıcı bilgilerini al (gerçek)
    const userAgent = navigator.userAgent
    const browser = detectBrowser(userAgent)
    const os = detectOS(userAgent)
    const device = detectDevice(userAgent)

    // Ekran bilgilerini al (gerçek)
    const screenResolution = `${window.screen.width}x${window.screen.height}@${window.devicePixelRatio}x`

    // Dil bilgilerini al (gerçek)
    const language = navigator.language

    // Zaman dilimi (gerçek)
    const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone

    // Donanım bilgilerini al (kısmen gerçek)
    const cpu = await detectCPU()
    const gpu = await detectGPU()
    const ram = await detectRAM()

    // PC adı ve anakart bilgilerini al (kısmen gerçek)
    const pcName = await detectPCName()
    const motherboard = await detectMotherboard()
    const macAddress = await detectMacAddress()

    // Ağ bilgilerini al (gerçek)
    const networkInfo = await getNetworkInfo()

    // Detaylı donanım bilgilerini al
    const hardwareInfo = await getDetailedHardwareInfo()
...
export const saveSystemInfo = async (userId?: string): Promise<void> => {
  try {
    const systemInfo = await getSystemInfo()

    // localStorage'a kaydet
    localStorage.setItem("roxen_system_info", JSON.stringify(systemInfo))

    // Kullanıcı bilgisiyle birlikte kaydet
    if (userId) {
      ...
      // Kullanıcı sistem bilgilerini kaydet
      const userSystemInfos = JSON.parse(localStorage.getItem("roxen_user_system_infos") || "[]")
      userSystemInfos.push(userSystemInfo)
      localStorage.setItem("roxen_user_system_infos", JSON.stringify(userSystemInfos))
    }
  } catch (error) {
...
export const getSavedSystemInfo = (): SystemInfo | null => {
  try {
    const systemInfoJson = localStorage.getItem("roxen_system_info")
    if (!systemInfoJson) return null

    return JSON.parse(systemInfoJson)
  } catch (error) {
    console.error("Kaydedilmiş sistem bilgileri alınamadı:", error)
    return null
  }
}
